''' Module contains all integration tests for redirect route '''
...
def test_redirect(client: TestClient):
    '''Test a valid redirect with an existing URLS,
    set allow_redirects to false so we can capture for test'''
...
def test_bad_redirect_long(client: TestClient):
    '''Test a valid request but with a short_url that is too long'''
